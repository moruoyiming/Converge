...
//    // 鱼的初始角度,0表示朝x轴正方向，90表示朝y轴负方向 private final float fishStartAngle = 90;
//    //鱼头圆的圆心
...
//        ValueAnimator valueAnimator = ValueAnimator.ofFloat(0, CHANGE_VALUE);// 变化 值
//        valueAnimator.setDuration(ANIMATOR_DURATION);// 设置动画周期 valueAnimator.setRepeatCount(ValueAnimator.INFINITE);// 设置循环次数，无限次 valueAnimator.setRepeatMode(ValueAnimator.RESTART);// 设置循环模式 valueAnimator.setInterpolator(new LinearInterpolator());// 默认插值器为
//        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { @Override
//        public void onAnimationUpdate(ValueAnimator animator) { currentValue = (float) animator.getAnimatedValue(); invalidateSelf();
//        } });
//        valueAnimator.start();
//    }
//
...
//// 身体底部的中心点
//        PointF bodyBottomCenterPoint = calculatPoint(headPoint, BODY_LENGHT, fishAngle - 180);
//// 节肢1
...
//                segmentAngle + 90);
//        PointF bottomRightPoint = calculatPoint(bottomCenterPoint, bigCircleRadius, segmentAngle - 90);
//        PointF upperLeftPoint = calculatPoint(upperCenterPoint, smallCircleRadius,
//                segmentAngle + 90);
//        PointF upperRightPoint = calculatPoint(upperCenterPoint, smallCircleRadius, segmentAngle - 90);
//        if (hasBigCircle) { // 画大圈
...
//                segmentAngle + 90);
//        PointF rightPoint = calculatPoint(centerPoint, triangleHalfLength, segmentAngle - 90);
//        // 绘制三角形
//        mPath.reset();
//        mPath.moveTo(startPoint.x, startPoint.y);
...
//                BIG_CIRCLE_RADIUS, fishAngle - 90);
//// 二阶贝塞尔曲线的控制点，决定鱼的胖瘦
//        PointF contralLeft = calculatPoint(headPoint, BODY_LENGHT * 0.56f, fishAngle + 130);
//        PointF contralRight = calculatPoint(headPoint, BODY_LENGHT * 0.56f, fishAngle - 130);
//        mPath.reset();
...
//                0f, 1f).setDuration(1000); rippleAnimator.start();
...
//        // 水波纹的透明度:从100到0
//        alpha = (int) (100 * (1 - radius)); // 水波纹的半径:从0到1变化
//        this.radius = radius;
//    }
//}
