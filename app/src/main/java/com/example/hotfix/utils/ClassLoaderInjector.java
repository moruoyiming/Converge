...
public class ClassLoaderInjector {
    ...
    private static ClassLoader createNewClassLoader(Context context, ClassLoader oldClassLoader, List<File> patchs) throws Throwable {
         ...
         */
        //得到原本的pathList
        Field pathListField = ShareReflectUtils.findField(oldClassLoader, "pathList");
        Object oldPathList = pathListField.get(oldClassLoader);

        //dexElements
        Field dexElementsField = ShareReflectUtils.findField(oldPathList, "dexElements");
        Object[] oldDexElements = (Object[]) dexElementsField.get(oldPathList);

        //从Element上得到 dexFile
        Field dexFileField = ShareReflectUtils.findField(oldDexElements[0], "dexFile");
        ...
        Field nativeLibraryDirectoriesField = ShareReflectUtils.findField(oldPathList, "nativeLibraryDirectories");
    ...
    private static void doInject(Application app, ClassLoader classLoader) throws Throwable {
        ...
        Context baseContext = (Context) ShareReflectUtils.findField(app, "mBase").get(app);
        if (Build.VERSION.SDK_INT >= 26) {
            ShareReflectUtils.findField(baseContext, "mClassLoader").set(baseContext, classLoader);
        }

        Object basePackageInfo = ShareReflectUtils.findField(baseContext, "mPackageInfo").get(baseContext);
        ShareReflectUtils.findField(basePackageInfo, "mClassLoader").set(basePackageInfo, classLoader);

        if (Build.VERSION.SDK_INT < 27) {
            Resources res = app.getResources();
            try {
                ShareReflectUtils.findField(res, "mClassLoader").set(res, classLoader);

                final Object drawableInflater = ShareReflectUtils.findField(res, "mDrawableInflater").get(res);
                if (drawableInflater != null) {
                    ShareReflectUtils.findField(drawableInflater, "mClassLoader").set(drawableInflater, classLoader);
                }
